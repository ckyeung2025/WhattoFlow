...
import { ClockCircleOutlined, MessageOutlined, FileTextOutlined, UserAddOutlined, MailOutlined } from '@ant-design/icons';
...
const OverdueEscalationModal = ({ 
  ...
  useEffect(() => {
    const handleTemplateSelected = (event) => {
      ...
      console.log('🎯 OverdueEscalationModal 收到模板選擇事件:', { template: template.name, isMetaTemplate });
      setSelectedTemplate({
        id: template.id,
        name: template.name,
        isMetaTemplate,
  ...
  useEffect(() => {
    if (visible) {
      console.log('🟡 OverdueEscalationModal useEffect - visible:', visible, 'initialConfig:', initialConfig);
      // 重置臨時 emailConfig
      ...
      if (initialConfig) {
        ...
        if ((initialConfig.messageMode === 'email' || initialConfig.emailConfig) && initialConfig.emailConfig) {
          console.log('🟡 OverdueEscalationModal 載入 email 配置:', initialConfig.emailConfig);
          setActiveTab('email');
  ...
  const handleSave = () => {
    ...
    if (activeTab === 'direct') {
      ...
      console.log('🟡 OverdueEscalationModal.handleSave - emailConfig:', finalEmailConfig);
      config = {
        ...config,
        useTemplate: false,
  ...
  const tabItems = [
    {
      ...
      children: (
        <DirectMessageTab
          ...
          mode="modal"
          description={t('workflowDesigner.overdue.escalationMessageDescription')}
          placeholder={t('workflowDesigner.overdue.escalationMessagePlaceholder')}
          rows={6}
          tip={t('workflowDesigner.overdue.escalationMessageTip')}
          value={directMessage}

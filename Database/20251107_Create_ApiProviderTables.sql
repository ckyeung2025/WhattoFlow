BEGIN TRY
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'ApiProviderDefinitions' AND schema_id = SCHEMA_ID('dbo'))
    BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'CompanyApiProviderSettings' AND schema_id = SCHEMA_ID('dbo'))
    BEGIN
    ...
    USING (VALUES
        ('AI', 'xai',      N'X.AI (Grok)',       N'grok',
         ...
         N'Elon AI Grok 官方 API',               1, 0.00, 1.50,
         N'apiKey', N'{"max_tokens":2000,"presence_penalty":0,"frequency_penalty":0,"stop":[]}'),

        ('AI', 'openai',   N'OpenAI',            N'openai',
         N'https://api.openai.com/v1/chat/completions',
         N'gpt-4o',        N'["gpt-4o","gpt-4.1-mini","gpt-4o-mini"]',
         N'OpenAI Chat Completions',             1, 0.00, 1.00,
         N'apiKey', N'{"max_tokens":2000,"presence_penalty":0,"frequency_penalty":0,"response_format":"text"}'),

        ('AI', 'deepseek', N'DeepSeek',          N'deepseek',
         N'https://api.deepseek.com/chat/completions',
         N'deepseek-chat', N'["deepseek-chat","deepseek-coder"]',
         N'DeepSeek 官方 Chat API',              1, 0.00, 1.50,
         N'apiKey', N'{"max_tokens":4096,"frequency_penalty":0,"presence_penalty":0,"stream_options":{"include_usage":false}}'),

        ('AI', 'copilot',  N'GitHub Copilot',    N'copilot',
         N'https://api.githubcopilot.com/v1/chat/completions',
         NULL,             NULL,
         N'Copilot Chat API（需企業授權）',         0, NULL, NULL,
         N'bearerToken', N'{"intent":"chat","n":1,"includeThoughts":false}'),
        ...
        ('AI', 'gemini',   N'Google Gemini',     N'gemini',
         ...
         N'Google AI Gemini API',                1, 0.00, 1.00,
         N'apiKey', N'{"maxOutputTokens":2048,"candidateCount":1,"topK":64,"stopSequences":[]}'),

        ('CloudDoc', 'google-docs', N'Google Docs API', N'google-docs',
         N'https://docs.googleapis.com/v1/documents',
         NULL, NULL,
         N'Google Docs 讀寫 API（Dataset 匯入、文件同步）', 0, NULL, NULL,
         N'serviceAccount', N'{"scopes":["https://www.googleapis.com/auth/documents","https://www.googleapis.com/auth/drive.file"],"applicationName":"WhatoFlow API Bridge"}')
    ) AS source (Category, ProviderKey, DisplayName, IconName, DefaultApiUrl, DefaultModel,
                 SupportedModels, Description, EnableStreaming, TemperatureMin, TemperatureMax,
